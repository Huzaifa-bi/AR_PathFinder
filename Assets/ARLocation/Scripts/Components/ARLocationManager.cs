...
namespace ARLocation
{
    ...
    public class ARLocationManager : Singleton<ARLocationManager>
    {
        ...
        private void Start()
        {
            ...
            if (!arSession)
            {
                Logger.LogFromMethod("ARLocationManager", "Start", "[AR+GPS][ARLocationManager#Start]: No ARSession found in the scene! Running in testing/offline mode without AR tracking.", DebugMode);
                SessionManager = null; // No AR session available for testing
                // Skip AR session initialization - app will work without AR tracking
            }
            else
            {
