...
namespace ARLocation.MapboxRoutes
{
    public class MapboxRoute : MonoBehaviour
    {
        ...
        public bool BuildRoute(RouteResponse result)
        {
            ...
            if (result == null)
            {
                Debug.LogError("[MapboxRoute#BuildRoute]: RouteResponse is null");
                s.LoadRouteError = "Route response is null. Check your Mapbox API token and network connection.";
                Settings.OnMapboxRouteLoadError?.Invoke(s.LoadRouteError);
                return false;
            }

            // Validate routes list exists and has items
            if (result.routes == null || result.routes.Count == 0)
            {
                Debug.LogError("[MapboxRoute#BuildRoute]: No routes found in response");
                s.LoadRouteError = "No valid routes returned from Mapbox API. Check coordinates and token permissions.";
                Settings.OnMapboxRouteLoadError?.Invoke(s.LoadRouteError);
                return false;
            }
            ...
            {
                Debug.LogError("[MapboxRoute#BuildRoute]: Route has no legs");
                s.LoadRouteError = "Route has no valid segments.";
                Settings.OnMapboxRouteLoadError?.Invoke(s.LoadRouteError);
                return false;
            }
            ...
            {
                Debug.LogError("[MapboxRoute#BuildRoute]: Leg has no steps");
                s.LoadRouteError = "Route leg has no valid steps.";
                Settings.OnMapboxRouteLoadError?.Invoke(s.LoadRouteError);
                return false;
            }
